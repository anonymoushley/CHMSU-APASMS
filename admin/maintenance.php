            ...
            case 'archive_building':
                ...
                if ($building_id && is_numeric($building_id)) {
                    try {
                        ...
                        $check_stmt = $pdo->prepare("SELECT id, name, status FROM buildings WHERE id = ?");
                        ...
                        if (!$building) {
                            ...
                            $stmt = $pdo->prepare("UPDATE buildings SET status = 'archived', updated_at = NOW() WHERE id = ?");
                ...
                if ($building_id && is_numeric($building_id)) {
                    try {
                        ...
                        if (!$building) {
                            ...
                            $stmt = $pdo->prepare("UPDATE buildings SET status = 'active', updated_at = NOW() WHERE id = ?");
                            $stmt->execute([$building_id]);
            ...
            case 'archive_room':
                ...
                if ($room_id && is_numeric($room_id)) {
                    try {
                        ...
                        $check_stmt = $pdo->prepare("SELECT r.id, r.room_number, r.status, b.name as building_name FROM rooms r JOIN buildings b ON r.building_id = b.id WHERE r.id = ?");
                        ...
                        if (!$room) {
                            ...
                            $stmt = $pdo->prepare("UPDATE rooms SET status = 'archived', updated_at = NOW() WHERE id = ?");
                ...
                if ($room_id && is_numeric($room_id)) {
                    try {
                        ...
                        if (!$room) {
                            ...
                            $stmt = $pdo->prepare("UPDATE rooms SET status = 'active', updated_at = NOW() WHERE id = ?");
                            $stmt->execute([$room_id]);
...
// Get active buildings for room form dropdown
try {
