...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_interview'])) {
    ...
    if ($reg_data) {
        ...
        $update_sql = "INSERT INTO screening_results (personal_info_id, interview_total_score, communication_skills, problem_solving, motivation, knowledge, overall_impression, interview_date)
                       VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
        ...
        $stmt->bind_param("iddddd", $personal_info_id, $total_score, $communication_skills, $problem_solving, $motivation, $knowledge, $overall_impression);
...
// Get applicants for interview (only those who are scheduled and not yet interviewed)
$sql = "SELECT 
        ...
        AND (sr.interview_total_score IS NULL OR sr.interview_total_score = 0)";
...
if (!empty($search)) {
    $stmt->bind_param("issssss", $search, $search_term, $search_term, $search_term, $search_term, $search_term, $search_term);
}
$stmt->execute();
                ...
                <div class="card-body">
                    <div class="input-group">
                        ...
                        <input type="text" id="searchInput" class="form-control" 
                               placeholder="Search by registration ID, name, strand, program, or campus..." 
                        ...
                        <?php if (!empty($applicants)): ?>
                            <?php foreach ($applicants as $applicant): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($applicant['strand']) ?: '-' ?></td>
                                    <td>
